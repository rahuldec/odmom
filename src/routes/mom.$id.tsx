    ...
    <AppShell>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" className="gap-1.5" onClick={() => downloadMomPdf(mom)}>
      ...
      <article className="space-y-6">
        <Card className="overflow-hidden">
          <div className="bg-slate-900 px-6 py-5 text-white dark:bg-slate-950">
        ...
        <p className="pt-2 text-center text-xs text-muted-foreground">
          Generated by Client Visit MOM Portal
        </p>
      </article>
    </AppShell>
  );
...
function PriorityBadge({ priority }: { priority: string }) {
  ...
  return <span className={`inline-flex h-fit shrink-0 rounded-md px-2 py-0.5 text-xs font-medium ${map[priority] ?? ""}`}>{priority}</span>;
}
